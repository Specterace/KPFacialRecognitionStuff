...
#take in a directory where images are stored but unprocessed, and for each image:
#take an image, extract a face, get it's embedding, store it's class/name
#Declare the path/file name of the directory in the call
inputDir = sys.argv[1]

if not os.path.exists('ProcessedPics/Identified/'):
    os.makedirs('ProcessedPics/Identified/')
if not os.path.exists('ProcessedPics/Unidentified/'):
    os.makedirs('ProcessedPics/Unidentified/')

for filename in listdir(inputDir):
    #path
    currPath = inputDir + filename
    print(currPath)
    #skip any directory files in the directory
    if isdir(currPath):
        continue
    ####skip any files that do not end in .jpeg or .jpg (will change for .png later)
    ####extract ending suffix in filename
    ####if ending suffix is not ".jpeg" or "jpeg", continue
    ...
    ####store the (claimed) face name (may need to be stored on the filename or on a .csv)
    ...
    #####move the image file to an appropriate folder, depending on a postive ID or not
    if (class_probability > 50.0):
        #####move image from the unprocessed dir to the dir associated with the ID'ed class
        ####can change the filepath to reflect renaming the image if wanted
        destPath = 'ProcessedPics/Identified/' + filename
        #destPath = 'kp-employees-dataset/' + predict_names[0] + '/' + filename
    else:
        #####move image from the unprocessed dir to the dir associated with a non-ID'ed class
        destPath = 'ProcessedPics/Unidentified/' + filename
        #destPath = 'NewMemberPics/' + input_face_name + '/' + filename
    print(destPath)
    shutil.move(currPath, destPath)
print('Inage Files Processed')
